...
def dtw_d(s1, s2, w):
    ...
    # INPUTS:
    # s1: signal 1, size  m * n. where m is the number of variables, n is the timesteps.
    # s2: signal 2, size  m * n. where m is the number of variables, n is the timesteps.
    ...
    s1 = np.expand_dims(np.asarray(s1), axis=0)
    s2 = np.expand_dims(np.asarray(s2), axis=0)
